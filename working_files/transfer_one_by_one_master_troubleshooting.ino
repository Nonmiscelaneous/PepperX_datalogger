...
uint8_t tx_buf[BUFFER_SIZE] {0xAA, 0xAA, 0xAA, 0xAA}; // initialize send buffer contents
uint8_t rx_buf[BUFFER_SIZE] {0, 0, 0, 0}; // initialize recieve buffer contents
uint32_t ck_speed = 100000; // SPI clock speed in Hz
uint16_t data_delay = 2; // delay between transmissions in miliseconds
...
void loop()
{
    ...
    delay(data_delay); // delay between transactions
}
